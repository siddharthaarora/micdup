...
// Minimal JSON helpers — no external library needed for this simple schema.
// We write/parse by hand to avoid pulling in a dependency.
...
std::wstring get_appdata_dir() {
    auto dir = get_known_folder(CSIDL_APPDATA) + L"\\MicDup";
    CreateDirectoryW(dir.c_str(), nullptr);
    return dir;
}

std::wstring get_models_dir() {
    auto dir = get_known_folder(CSIDL_LOCAL_APPDATA) + L"\\MicDup\\Models";
    std::filesystem::create_directories(dir);
    return dir;
}

static std::wstring settings_path() {
    return get_appdata_dir() + L"\\settings.json";
}
...
AppSettings settings_load() {
    ...
    if (!mods_str.empty()) {
        ...
        if (mods_str.find("Alt")     != std::string::npos) m |= MOD_ALT;
        if (mods_str.find("Win")     != std::string::npos) m |= MOD_WIN;
...
void settings_save(const AppSettings& s) {
    ...
    if (s.hotkey.modifiers & MOD_ALT)     { if (!mods.empty()) mods += "+"; mods += "Alt"; }
    if (s.hotkey.modifiers & MOD_WIN)     { if (!mods.empty()) mods += "+"; mods += "Win"; }
...
std::string format_modifiers(uint32_t mods) {
    ...
    if (mods & MOD_ALT)     { if (!r.empty()) r += " + "; r += "Alt"; }
    if (mods & MOD_WIN)     { if (!r.empty()) r += " + "; r += "Win"; }
    return r;
}
